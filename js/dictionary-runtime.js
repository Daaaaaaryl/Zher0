...
function canonicalDictionaryReferences(gameData){
  return {
...
function validateDictionaryData(dictionaryData,gameData){
  const errors=[];
  const duplicateIds=[];
  ...
  const canonical=canonicalDictionaryReferences(gameData);
  const entryIndex=new Map();
  const referenceIndex=new Map();
  const counts={stat:0,equipment_slot:0,equipment_set:0,equipment:0};
...
function assertValidDictionaryData(dictionaryData,gameData){
  const result=validateDictionaryData(dictionaryData,gameData);
  if(!result.valid){
...
async function loadDictionary(options={}){
  ...
  DICTIONARY_VALIDATION=deepFreezeDictionary(assertValidDictionaryData(source,gameData));

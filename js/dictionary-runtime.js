...
function canonicalDictionaryReferences(gameData,monsterData=null,abilityData=null,statusData=null,damageTypeData=null){
  return {
    ...
    damage_type:new Set((damageTypeData?.damageTypes||[]).map(item=>item.id))
  };
}

function validateDictionaryData(dictionaryData,gameData,monsterData=null,abilityData=null,statusData=null,damageTypeData=null){
  const errors=[];
  const duplicateIds=[];
  ...
  const canonical=canonicalDictionaryReferences(gameData,monsterData,abilityData,statusData,damageTypeData);
  const entryIndex=new Map();
  const referenceIndex=new Map();
  const counts={stat:0,equipment_slot:0,equipment_set:0,equipment:0,monster_family:0,monster_role:0,monster_tier:0,monster:0,ability:0,status_effect:0,damage_type:0};
...
function assertValidDictionaryData(dictionaryData,gameData,monsterData=null,abilityData=null,statusData=null,damageTypeData=null){
  const result=validateDictionaryData(dictionaryData,gameData,monsterData,abilityData,statusData,damageTypeData);
  if(!result.valid){
...
async function loadDictionary(options={}){
  ...
  if(!damageTypeData)throw new Error("Canonical damage type data must be loaded or supplied before the dictionary");
  const response=await fetch(path,{cache:"no-store"});
  if(!response.ok)throw new Error(`Unable to load ${path}: HTTP ${response.status}`);
  const source=await response.json();
  DICTIONARY_VALIDATION=deepFreezeDictionary(assertValidDictionaryData(source,gameData,monsterData,abilityData,statusData,damageTypeData));
